...
from gnn_setup.setups.data import make_ogbn_dataset_splits, load_ogbn_dataset_splits
...
def default_config():
    ...
    wandb_flag = True
    wandb_project = "ICLR-products-attack"
...
def run(dataset_name, model_name, attack_name, n_runs, inductive,
    ...
    default_attack_configs = yaml.safe_load(open("./conf/attack-configs.yaml")).get("products_configs")
    ...
    splits_root += "/product/"
    models_root += "/product/"
    reports_root += "/product/"


    assert dataset_name == "ogbn-products"
    ...
    for split in tqdm(dataset_splits):
        ...
        dataset = load_ogbn_dataset_splits(
