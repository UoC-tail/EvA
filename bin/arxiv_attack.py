...
def run(dataset_name, model_name, attack_name, n_runs, inductive,
    ...
    for split in tqdm(dataset_splits):
        ...
        # pert_adj = attack_instance["pert_adj"]
        # test_adj = dataset["test_adj"]
        # temp = (pert_adj - test_adj).coalesce().to('cpu')
        # idx = torch.where(temp.values() ==1)
        # indices = temp.indices()
        
        # import numpy as np
        # # import pdb; pdb.set_trace()
        # # random pick n nodes from test_idx
        # n = 1000
        # # each run sample different nodes from test set
        # np.random.seed(0)
        # sub_test_idx = np.random.choice(test_idx, n, replace=False)
        # sub_test_idx.sort()
        # sub_test_idx = torch.tensor(sub_test_idx)
