...
from gnn_setup.utils.ppr_utils import ppr_topk
...
def calculate_loss(logits, labels, loss_type):
        ...
        if loss_type == 'CW':
            ...
            loss = -torch.nn.LeakyReLU(negative_slope=0.1)(margin).mean()
...
def count_edges_for_idx(adj, idx):
    ...
    if isinstance(adj, SparseTensor):
        ...
        row, col = adj.indices()[0].to(device), adj.indices()[1].to(device)
...
def count_edge_for_inter_subset(adj, idx_g, idx_s):
    ...
    mapping_g = torch.zeros(adj.size(dim=0)).bool().to(device)
    mapping_g[idx_s]=True # True if node in idx
  
    mapping_s = torch.zeros(adj.size(dim=0)).bool().to(device)
    mapping_s[idx_g]=True # True if node in idx
    
    mask_row = mapping_s[row] # True if row in idx
    mask_col = mapping_g[col] # True if col in idx
    
    mask_row_col = torch.logical_and(mask_col, mask_row) # True if either row or col in idx -> edges connected to idx
    if idx_s.shape == idx_g.shape and (idx_s == idx_g).all():
        return mask_row_col.sum()
    return mask_row_col.sum() - count_edge_for_inter_subset(adj, idx_s, idx_s)