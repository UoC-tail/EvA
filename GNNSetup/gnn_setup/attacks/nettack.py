...
from gnn_setup.gnns import MODEL_TYPE
from gnn_setup.utils.robust_training_utils import sparse_tensor
from gnn_setup.attacks.base_attack import SparseLocalAttack
...
class Nettack(SparseLocalAttack):
    ...
    #    device='cuda:0', size=(2810, 2810), nnz=15962, layout=torch.sparse_coo)
        ...
        size = self.adj.size()
        self.sp_adj = sp.csr_matrix((values.cpu().numpy(), (indices[0].cpu().numpy(), indices[1].cpu().numpy())),shape=size)
