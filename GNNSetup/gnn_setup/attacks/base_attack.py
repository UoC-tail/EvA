...
from gnn_setup.gnns import MODEL_TYPE, DenseGCN, DenseGPRGNN, GCN, RGNN
...
class Attack(ABC):
    ...
    def get_pertubations(self):
        ...
        if isinstance(self.adj_adversary, torch.Tensor) and self.adj_adversary.is_sparse == False:
...
class SparseAttack(Attack):
    ...
    def __init__(self,
        ...
        if isinstance(adj, torch.Tensor) and adj.is_sparse == False:
        ...
        if isinstance(adj, SparseTensor):
            ...
            self.edge_index = adj.indices().to(self.data_device)
            self.edge_weight = adj.values().to(self.data_device)
        self.n = adj.size(0)
