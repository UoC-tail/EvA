...
class EvAttackAcceleratedMultiObj(EvAttack):
    ...
    def find_optimal_perturbation(self, n_perturbations: int, **kwargs):
        ...
        self.problem = GraphBatchProblem(
            objective_sense=['min' for i in range(90)],
    ...
    def batch_evaluation(self, attr_batch, adj_batch, labels_batch, model, mask_attack):
        ...
        num_evals = 89
        mask_size = math.ceil(len(idx_attack) / num_evals)
        all_maskes = [torch.zeros_like(mask_attack) for _ in range(num_evals)]
        for i in range(num_evals):
            if i == num_evals - 1:
                all_maskes[i][idx_attack[i*mask_size:]] = 1
            else:
                all_maskes[i][idx_attack[i*mask_size:(i+1)*mask_size]] = 1

        hits_masked = hits[:, mask_attack]
        all_acc = hits_masked.float().mean(dim=1)

        accs = [all_acc]
        accs.extend([hits[:, mask].float().mean(dim=1) for mask in all_maskes])

        accs = torch.stack(accs, dim=1)
        return accs
