...
class EvAttackAcceleratedCE(EvAttack):
    ...
    def batch_evaluation(self, attr_batch, adj_batch, labels_batch, model, mask_attack):
        ...
        logits = logits.reshape(-1, self.n_nodes, logits.shape[-1])
        
        base_logits = model(self.attr, self.adj)
        sorted_arg = base_logits.argsort(-1)


        criterion = nn.CrossEntropyLoss(reduction='none')
        logits_reshaped = logits.view(-1, logits.shape[-1])  # Shape [512 * 2810, 7]
        labels_repeated = labels_batch.unsqueeze(0).repeat(logits.shape[0], 1).view(-1)  # Shape [pop * 2810]
        losses = criterion(logits_reshaped, labels_repeated)  # Shape [numpup * 2810]
        losses_reshaped = losses.view(logits.shape[0], logits.shape[1])
        loss = -losses_reshaped.mean(dim=1)  # Shape [512]
